...
#		help="trimmomatic stat info file list, Name\tstat, same order as log list")
	ars("--input_dir", dest="input_dir", required=True,
		help="input dir that contains all logs files")
	ars("--bms_list", dest="bms_list", required=True,
		help="bms list file that contain all samples info")
	...
	ars = g.add_argument
	ars("--chunk", dest="chunk", default=10000000, type=int,
...
if __name__ == "__main__":
	...
	log_dict, stats_dict = {}, {}
	with open(pars['out_stat_file'], "w") as ot:
		ot.write("SampleName\tRawReadPair\tCleanReadPair\tCleanForwardRead\tCleanReverseRead\t"
				 "CleanReadPair/RawReadPair(%)\tTotalCleanRead/RawRead(%)\tCleanBP/RawBP(%)\n")
...
#		with nested(open(log_list), open(stat_list)) as (logs, stats):
			for log in logs:
				...
				clean_bp = int(sum(bp_info.ix[:,len(bp_info.columns)-3]))
